...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: '/event3', label: 'Electrica Quest' },
    { path: '/event4', label: 'Circuit Debugging' },
    { path: '/event5', label: 'Ohm-azing Quiz' },
    { path: '/event6', label: 'Surprise Event' },
    { path: '/map', label: 'Map' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between md:justify-center h-16"> {/* justify-between for mobile, justify-center for desktop */}
          <Link to="/" className="flex items-center space-x-2 absolute left-4 md:relative md:left-auto"> {/* absolute positioning for mobile logo */}
            <Sparkles className="h-8 w-8 text-teal-green-500" />
            <span className="text-xl font-bold text-teal-green-100 whitespace-nowrap">
              BlitzKrieg 2K25
            </span>
          </Link>
          ...
          <div className="hidden md:flex md:items-center space-x-8">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className="text-gray-300 hover:text-teal-green-100 px-3 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap"
              >
                {item.label}
              </Link>
            ))}
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-black/95"> {/* Updated background color here */}
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className="text-gray-300 hover:text-teal-green-100 block px-3 py-2 rounded-md text-base font-medium text-left" // Align text to left
